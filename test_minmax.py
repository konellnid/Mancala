...
class MinMaxTest(unittest.TestCase):
    def test_choosing_right_capture(self):
        min_max_player = MinMaxPlayer(2, DEFAULT_EVALUATION_FUNCTION)
        # choosing move 4 allows to capture 15 seeds
        board = [1, 0, 1, 0, 1, 0, 2,
                 15, 1, 1, 1, 1, 1, 7]
        position = MancalaPosition(board, IS_PLAYER_A_MOVE)

        chosen_move = min_max_player.choose_move(position, position.get_possible_moves())

        self.assertEqual(chosen_move, 4)

    def test_avoiding_getting_captured(self):
        min_max_player = MinMaxPlayer(3, DEFAULT_EVALUATION_FUNCTION)
        ...
        if chosen_move == 5 and len(min_max_player.current_move_sequence) > 0:
